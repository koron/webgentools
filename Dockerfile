...
FROM debian:bookworm-20251020-slim AS protoc-extract
ARG TARGETARCH
ARG PROTOC_VER
WORKDIR /opt
COPY arch.sh .
...
FROM debian:bookworm-20251020-slim AS go-extract
ARG TARGETARCH
ADD https://go.dev/dl/go1.25.3.linux-${TARGETARCH}.tar.gz /opt/go.tar.gz
